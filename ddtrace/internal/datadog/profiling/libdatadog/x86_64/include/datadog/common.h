...
#define DDOG_CHARSLICE_C(string)                                                                                       \
    /* NOTE: Compilation fails if you pass in a char* instead of a literal */ {                                        \
        .ptr = "" string, .len = sizeof(string) - 1                                                                    \
    }
#else
#define DDOG_CHARSLICE_C(string)                                                                                       \
    /* NOTE: Compilation fails if you pass in a char* instead of a literal */ (                                        \
      (ddog_CharSlice){ .ptr = "" string, .len = sizeof(string) - 1 })
#endif
 ...
 */
void
ddog_Error_drop(struct ddog_Error* error);
 ...
 */
ddog_CharSlice
ddog_Error_message(const struct ddog_Error* error);

DDOG_CHECK_RETURN struct ddog_Vec_Tag
ddog_Vec_Tag_new(void);
 ...
 */
DDOG_CHECK_RETURN
struct ddog_Vec_Tag_PushResult
ddog_Vec_Tag_push(struct ddog_Vec_Tag* vec, ddog_CharSlice key, ddog_CharSlice value);
 ...
 * .len property.
 */
DDOG_CHECK_RETURN struct ddog_Vec_Tag_ParseResult
ddog_Vec_Tag_parse(ddog_CharSlice string);
